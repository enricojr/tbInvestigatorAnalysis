...
int bumplet(const char *fileNameIn,
  ...
  TFile *fileOut = TFile::Open("tmp.root", "RECREATE");
  if(!fileOut){
  ...
  while(!endOfFile){
	...
	if(iDRS == cfg -> _resetCH.first && iCH == cfg -> _resetCH.second){
	  ...
	  fitWaveform -> reset();	  
	  if(pulseAmplitude != 0 && pulseRiseTime>8) countHits++;
	}
	...
	h1[i] = new TH1F(h1Name, h1Name, 1024, 0, 1024);
	for(unsigned int j=0; j<1024; j++){
	  h1[i] -> Fill(j, WAVEFORM[i][j]);
	}
      }
      char ccName[1000];
      sprintf(ccName, "tmp/evt%08d.png", eventNumber);
